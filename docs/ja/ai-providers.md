...
MiniMax は Anthropic 互換 API を使用しており、インターリーブ思考などの機能に推奨されています。
...
任意のカスタムエンドポイント（デフォルトは国際エンドポイント）:
...
[MiniMax プラットフォーム](https://platform.minimax.chat/)から API キーを取得してください。
