...
/**
 * DynamoDB-based quota manager implementation.
 * Uses composite key (PK=user, SK=date) for per-day tracking.
 * Quota tracking is OPT-IN: only enabled if DYNAMODB_QUOTA_TABLE is set.
 */
export class DynamoQuotaManager extends BaseQuotaManager {
    ...
    constructor() {
        super()
        this.table = TABLE
        this.quotaTimezone = QUOTA_TIMEZONE
    ...
    async checkAndIncrementRequest(
        ...
        const pk = user // User identifier (base64 IP)
        const sk = this.getTodayInTimezone() // Date as sort key (YYYY-MM-DD)
        ...
        try {
            ...
            if (e instanceof ConditionalCheckFailedException) {
                ...
                try {
                    ...
                    console.warn(
                        `[quota] Condition failed but no limit exceeded for IP prefix: ${user.slice(0, 8)}...`,
                    )
                    return { allowed: true }
                } catch (getError: any) {
                    console.error(
                        `[quota] Failed to get quota details after condition failure, IP prefix: ${user.slice(0, 8)}..., error: ${getError.message}`,
                    )
            ...
            console.error(
                `[quota] DynamoDB error (fail-open), IP prefix: ${user.slice(0, 8)}..., error: ${e.message}`,
            )
            return { allowed: true }
        }
    }
     ...
     */
    async recordTokenUsage(user: string, tokens: number): Promise<void> {
        // Skip if quota tracking not enabled
        if (!this.client || !this.table) return
        ...
        try {
            ...
            if (e instanceof ConditionalCheckFailedException) {
                ...
                try {
                    ...
                    console.error(
                        `[quota] Failed to record tokens (retry), IP prefix: ${user.slice(0, 8)}..., tokens: ${tokens}, error: ${retryError.message}`,
                    )
                }
            } else {
                console.error(
                    `[quota] Failed to record tokens, IP prefix: ${user.slice(0, 8)}..., tokens: ${tokens}, error: ${e.message}`,
                )
            }
        }
    }
...
export const quotaManager = new DynamoQuotaManager()
